...
//点击添加或编辑的窗口的确定按钮时，提交预约信息
function addOrEdit() {
    //获取表单中预约id的内容
    var ebid = $("#ebid").val();
    //如果表单中有预约id的内容，说明本次为编辑操作
    if (ebid > 0) {
...
function returnBook(bid) {
    var r = confirm("确定离开场所?");
    var c=getProjectPath();
    console.log(c);
...
//页面加载完成后，给预约模态窗口的输入框绑定失去焦点和获取焦点事件
$(function() {
...
function changePage(pageNo,pageSize) {
    ...
    //如果跳转的是预约记录查询的相关链接，提交预约记录查询栏中的参数
